...
public class GifIndexedHandler extends GifHandlerBase {
	...
	/**
	 * Prepare decoding the current image. Create a bufferedimage and create the
	 * color table belonging to it from the global or local color table.
	 */
	@Override
	protected BufferedImage prepare() throws java.io.IOException {
		// -- Prepare the color model,
		if (m_im.m_transparant)
			m_icm = new IndexColorModel(m_im.m_bits_colortable,
					m_im.m_sz_colortable, m_im.m_reds, m_im.m_grns,
					m_im.m_blus, m_im.m_transparant_ix);
		else
			m_icm = new IndexColorModel(m_im.m_bits_colortable,
					m_im.m_sz_colortable, m_im.m_reds, m_im.m_grns, m_im.m_blus);
