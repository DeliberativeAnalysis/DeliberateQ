...
public class AnimGifDecoder {
	...
	/**
	 * Returns T if the stream (seems) to represent a GIF file. Checks for the
	 * GIF8xa marker.
	 */
	...
	private void createHandler(int bufimatype) throws IOException {
		...
		if (m_bi == null)
			throw new IOException(
					"Unexpected format of recognised BufferedImage type!?");
	}
	...
	private void readImageMeta() throws IOException {
		...
		for (;;) {
			...
			switch (blocktype) {
			default:
				throw new IOException("AnimGifDecoder: unexpected block type "
	 ...
	 * BufferedImage type and handles the writing of pixels in the most optimal
	 * way. The GifImageHandler is a base class that knows how to move from
	 * pixel to pixel. Derived classes know how to write every BufferedImage
	 * type that is supported. Since each derived class knows the exact format
	 * of the databuffer e.a. it can write the data as quick as possible without
	...
	private int getCode() throws IOException {
		...
		while (m_bitpos >= 8 && !m_lastblock) {
			...
			m_32bits |= m_block[m_block_ix++] << 24; // Shift in new byte at the
														// top
		}
		return code;
	}
	 ...
	 */
	private void initStringTable(int[] prefix, byte[] suffix, byte[] init,
			int[] length) {
		...
		// -- Fill the rest of the table to be prepared against out-of-sequence
		// codes,
