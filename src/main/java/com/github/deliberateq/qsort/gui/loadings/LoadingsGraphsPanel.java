...
public class LoadingsGraphsPanel extends JPanel {
    ...
    public void setRotations(final Rotations rotations) {
        ...
        if (graphPanels.size() == 0) {
            ...
            int numGraphs = loadings.columnCount() * (loadings.columnCount() - 1) / 2;
                ...
                for (int j = i + 1; j <= loadings.columnCount(); j++) {
                        ...
                        public void keyPressed(KeyEvent e) {
                            if (e.getKeyCode() == 37 || e.getKeyCode() == 39) {
                                if (gp != null) {
                                    ...
                                    if (e.getKeyCode() == 37)
                                        direction = 1;
                                    rotations.addRotation(new MatrixRotation(map.get(gp).x, map.get(gp).y, direction));
                    ...
                    gp.addFocusListener(new FocusListener() {
                        ...
                        public void focusGained(FocusEvent e) {
                            if (currentGp != null)
                                currentGp.setBorder(BorderFactory.createEmptyBorder());
                            currentGp = gp;
                            updateSelection();
                        }
    ...
    private void updateSelection() {
        if (currentGp != null)
            currentGp.setBorder(BorderFactory.createEtchedBorder(Color.black, Color.lightGray));
        repaint();
    }
